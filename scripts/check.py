...
# 2026-08-06: 英文レジュメ_構成案・LinkedIn更新案・REVIEW* は提出完了後 documents/output/{接尾辞}/md/_archive/
...
# `documents/output/{接尾辞}/md/` には、提出書類の正本に加えて REVIEW・事実性監査・ATS検証
# のような補助記録も置く。PDFビルダーが生成対象にする4種類だけを、md↔pdf整合性の対象にする。
# ファイル名の契約は build_pdf.py の _md_paths() と一致させる（未知の補助mdを「PDF未生成」と
# 誤判定しないため、プレフィックス除外ではなく許可リストにする）。
SUBMISSION_MD_PATTERNS = (
    re.compile(r"^履歴書_.+_(?:ja|en)\.md$"),
    re.compile(r"^職務経歴書_.+_ja\.md$"),
    re.compile(r"^Resume_.+_en\.md$"),
)
...
def check_triplet(outdir, label):
    ...
    mds = [m for m in mds
           if any(p.fullmatch(os.path.basename(m)) for p in SUBMISSION_MD_PATTERNS)]
    if not mds:
...
def check_chronology():
    ...
    # 2026-08-10: 応募先ごとのサブフォルダ化に伴い documents/output/{接尾辞}/md/ を走査する
...
def check_status_sync():
    ...
    ままの記載）が発覚したため、機械照合できるようマーカーコメントを導入した（教訓2・24）。"""
...
if __name__ == "__main__":
    # 2026-08-10: 応募先ごとのサブフォルダ化。応募先ごとに md/pdf の対を検証する。
    company_dirs = sorted(d for d in glob.glob(os.path.join(DOC_OUT, "*"))
                          if os.path.isdir(d) and os.path.isdir(os.path.join(d, "md")))
    if not company_dirs:
        warn.append("応募書類: documents/output/{接尾辞}/md/ が見つかりません")
    for d in company_dirs:
        check_triplet(d, f"応募書類/{os.path.basename(d)}")
    check_chronology()
    check_input()
    check_expiry()
    check_status_sync()
