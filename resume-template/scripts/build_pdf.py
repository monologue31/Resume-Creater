...
# ============================================================ md 読み込み・パース
# documents/output/md/ を単一の情報源（single source of truth）とする。
...
# 従来は特定の接尾辞（例: "会社名_職種"）が RIREKISHO_MD 等の定数に直接埋め込まれており、他社の接尾辞では
# 動かなかった。`_md_paths(suffix)` に組み立てロジックを1箇所へ集約し、接尾辞は
# `documents/output/md/` に実在するファイルから自動検出する（`_discover_suffixes()`）。
# ⚠️ BACKLOG.md項目1が提案していた案（input/apply/07_応募先.mdから接尾辞を読み取る）ではなく
# 実在ファイルからの自動検出にした理由: `07_応募先.md`は応募先を切り替えるたびに内容が
# 上書きされる単一の作業ファイルであり（過去の応募先の接尾辞情報が残っている保証がない）、
# 複数応募先が同時に`documents/output/md/`に存在する状態（2社目の応募先を追加するような場面）を
# 正しく扱えない。生成済みファイルそのものを単一の情報源にする方が、設定と実体がズレるリスクが無い。
def _md_paths(suffix):
    """接尾辞（例: "会社名_職種"）から4書類のmdパスを組み立てる。
    ファイル名の組み立てロジックはここに集約——他社対応時に触る場所はここだけでよい。"""
    return dict(
        rirekisho=os.path.join(OUT_MD, f"履歴書_{suffix}_ja.md"),
        shokumu=os.path.join(OUT_MD, f"職務経歴書_{suffix}_ja.md"),
        resume_en=os.path.join(OUT_MD, f"Resume_{suffix}_en.md"),
        rirekisho_en=os.path.join(OUT_MD, f"履歴書_{suffix}_en.md"),
    )


def _discover_suffixes():
    """documents/output/md/ に実在する4種のファイル名パターンから接尾辞を自動検出する。
    4パターンのいずれかにでも存在すれば拾う（英文レジュメのみ先行、等の状態にも対応するため）。"""
    patterns = [
        (os.path.join(OUT_MD, "履歴書_*_ja.md"), r"^履歴書_(.+)_ja\.md$"),
        (os.path.join(OUT_MD, "職務経歴書_*_ja.md"), r"^職務経歴書_(.+)_ja\.md$"),
        (os.path.join(OUT_MD, "Resume_*_en.md"), r"^Resume_(.+)_en\.md$"),
        (os.path.join(OUT_MD, "履歴書_*_en.md"), r"^履歴書_(.+)_en\.md$"),
    ]
    suffixes = set()
    for glob_pat, name_re in patterns:
...
def _extract_name(text):
    """先頭部分（最初の `## `より前）にある「氏名: ...」を取り出す。
    職務経歴書の見出し直下に「氏名: {お名前}」の行を置く運用を前提とする
    （テンプレート化にあたり追加。以前はbuild_shokumu()内に氏名が直接埋め込まれていた）。"""
    head = _re.split(r"^## ", text, maxsplit=1, flags=_re.M)[0]
    m = _re.search(r"^氏名[:：]\s*(.+)$", head, _re.M)
    return m.group(1).strip() if m else "{氏名未設定}"
...
# 2026-08-06追加（詳細はARCHITECTURE.md参照）。
#
...
SHOKUMU_H2 = {"職務要約", "活かせる経験・知識・スキル", "職務経歴", "マネジメント経験", "語学",
             "自己PR", "テクニカルスキル", "保有資格"}
# 保有資格の直下(H3)。「主要認定」は完全一致。「ServiceNow 認定」「Microsoft 認定」はprefix一致
# （サブモジュール名が変わっても対応できるよう意図的にprefix一致にしている）。
# 「対応中の上位資格」は無くてもよい任意セクション（CRMA/CTA等、結果待ちの資格が無い時期は存在しない）。
SHOKUMU_CERT_H3_EXACT_REQUIRED = {"主要認定"}
SHOKUMU_CERT_H3_PREFIX_REQUIRED = ["ServiceNow 認定", "Microsoft 認定"]
...
def _assert_h3_set(sub, expected, label):
    ...
    if extra:
        raise RuntimeError(f"{label}: 未知の見出し(###)が見つかりました: {sorted(extra)}"
                           f"\n  → build_pdf.py側の対応表とload_*()を更新してください。")


def _assert_cert_subs(cert_subs, label="職務経歴書/保有資格"):
    """保有資格直下(H3)は「完全一致の必須」「prefix一致の必須」「prefix一致の任意」が混在するため、
    _assert_h3_set とは別の判定ロジックを使う。未知の見出し（任意リストにも無いもの）は
    fail-loudの対象——これが「対応中の上位資格」取りこぼし事故の再発防止そのもの。"""
    got = set(cert_subs.keys())
    consumed = set()

    missing = SHOKUMU_CERT_H3_EXACT_REQUIRED - got
    consumed |= SHOKUMU_CERT_H3_EXACT_REQUIRED & got

    for prefix in SHOKUMU_CERT_H3_PREFIX_REQUIRED:
        hits = {k for k in got if k.startswith(prefix)}
        if not hits:
            missing.add(f"{prefix}...")
        consumed |= hits

    for prefix in SHOKUMU_CERT_H3_PREFIX_OPTIONAL:
        consumed |= {k for k in got if k.startswith(prefix)}

    if missing:
        raise RuntimeError(f"{label}: 必須の見出しが見つかりません: {sorted(missing)}")
    unknown = got - consumed
    if unknown:
        raise RuntimeError(f"{label}: 未知の見出しが見つかりました: {sorted(unknown)}"
                           f"\n  → 新しい資格区分を追加したなら、SHOKUMU_CERT_H3_*（build_pdf.py冒頭）と"
                           f"load_shokumu()の両方を更新してください。")
...
def load_rirekisho(suffix):
    ...
    lic_note = None
    m = _re.search(r"^※\s*(.+)$", secs["免許・資格"], _re.M)
    if m:
        lic_note = m.group(1).strip()
    ...
    return dict(date=date, basic=basic, gaku=gaku_rows, shoku=shoku_rows, lic=lic_rows,
                lic_note=lic_note, pr=pr_paras, commute=commute, shobatsu=shobatsu, hope=hope)
...
def load_shokumu(suffix):
    ...
    # ⚠️ テンプレート化時の修正: 元プロジェクトではここに「会社数は必ず3件」という
    # assert があったが、実際の描画（build_shokumu()側）は for i, c in enumerate(...) で
    # 会社数に依存しない汎用ループになっており、assertの方が古い制約を引きずった
    # 死んだ検証コードだった。会社数は人によって異なるため、
    # 最低1社あることだけを検証する形に修正した。
    if len(companies) < 1:
        raise RuntimeError(
            f"職務経歴書/職務経歴: 会社セクションが1件も見つかりません。"
            f"「## 職務経歴」の下に「### 会社1: ...」のような見出しがあるか確認してください。")
    ...
    _assert_cert_subs(cert_subs)
    cert_main_rows = _table_rows(cert_subs["主要認定"])[1:]
    # ⚠️ 2026-08-07: prefix文字列のハードコード重複を解消（BACKLOG.md項目2）。
    # 従来はここに "ServiceNow 認定" 等の文字列リテラルを再度書いており、_assert_cert_subs()が
    # 参照する SHOKUMU_CERT_H3_PREFIX_REQUIRED と2箇所で同じ文字列を維持する必要があった
    # （見出し名を変える際、片方だけ直して事故る典型パターン）。以後はこの定数のみを参照する。
    sn_heading = [k for k in cert_subs if k.startswith(SHOKUMU_CERT_H3_PREFIX_REQUIRED[0])][0]
    sn_groups_raw = _re.findall(r"\*\*(.+?)\*\*\n\n(.+?)(?=\n\n\*\*|\Z)",
                                cert_subs[sn_heading].strip() + "\n\n**", _re.S)
    sn_groups = [(f"ServiceNow — {h.strip()}", _strip_trailing_rule(b)) for h, b in sn_groups_raw]
    ms_heading = [k for k in cert_subs if k.startswith(SHOKUMU_CERT_H3_PREFIX_REQUIRED[1])][0]
    ms_body = _strip_trailing_rule(cert_subs[ms_heading].strip())
    # md 末尾の「以上」がファイル終端の区切りとして最終セクションの本文に紛れ込むため除去
    # （文末の「以上」は build_shokumu() が別途固定で描画するため、ここで残すと二重表示になる）
    ms_body = _re.sub(r"\n*以上\s*$", "", ms_body).strip()

    # 対応中の上位資格（結果待ち・受験予定）（CRMA・CTA等、「取得済み」と誤読されないよう主要認定・ServiceNow認定とは
    # 別区分でレンダリングする。2026-08-05追加。cert_subsに拾われるだけで終わらせず、
    # 明示的にdata dictへ渡してbuild_shokumu()側で描画すること）
    learning_keys = [k for k in cert_subs if k.startswith(SHOKUMU_CERT_H3_PREFIX_OPTIONAL[0])]
    learning_heading = learning_keys[0] if learning_keys else None
    learning_body = None
    if learning_heading:
        learning_body = _strip_trailing_rule(cert_subs[learning_heading].strip())
        learning_body = _re.sub(r"\n*以上\s*$", "", learning_body).strip()
    ...
    return dict(date=date, name=name, summary=summary, skill_items=skill_items, companies=companies,
                ...
                cert_main_rows=cert_main_rows, sn_heading=sn_heading, sn_groups=sn_groups,
                learning_heading=learning_heading, learning_body=learning_body,
                ms_heading=ms_heading, ms_body=ms_body, pr=pr_paras)
...
def load_rirekisho_en(suffix):
    ...
    lic_note = None
    m = _re.search(r"^※\s*(.+)$", secs["Licenses / Qualifications"], _re.M)
    if m:
        lic_note = m.group(1).strip()
...
def build_rirekisho(path, suffix):
    ...
    # ⚠️ テンプレート化時の修正: lic_note（免許・資格欄の「※」注記）は任意項目であり
    # 無いのが自然な状態（元プロジェクトはたまたま注記があったため、無い場合の分岐が
    # 抜けていた）。無い場合はこの段落自体を出力しない。
    if data["lic_note"]:
        st.append(P(f"※ {_bold(data['lic_note'])}", SMALL))
...
def build_rirekisho_en(path, suffix):
    ...
    st += [t3, Spacer(1, 1.5 * mm)]
    if data["lic_note"]:
        st += [P(f"※ {_bold(data['lic_note'])}", SMALL)]
    st += [Spacer(1, 3 * mm)]
...
def build_shokumu(path, suffix):
    ...
    st += [Paragraph("職 務 経 歴 書", TITLE), Spacer(1, 1.5 * mm),
           Paragraph(f"{data['date']}<br/>氏名: {data['name']}", META)]
    ...
    # ⚠️ テンプレート化時の修正: 元は会社を companies[0]/[1]/[2] のように位置で決め打ちし、
    # ベンダー表・備考(note)の描画も特定の1社にしか適用されていなかった（会社が正確に3社の
    # ときしか正しく動かない状態だった）。会社数、どの会社にベンダー表や備考があるかは人に
    # よって異なるため、companies を汎用的にループし、ベンダー表・備考の有無はそれぞれの
    # データ（case["vendor"]・company["note"]）から判定する形に修正した。

    def vendor_table(vendor_rows):
        # repeatRows=1: 表が途中で改ページされた場合もヘッダ行（ベンダー／製品／領域）を
        # 次ページ先頭に繰り返す（表を跨ぐ際にヘッダの無い続きが表示される事故を防ぐ）
    ...
    for idx, company in enumerate(data["companies"]):
        if idx == 0:
            st += band("職務経歴", keep_with=Paragraph(company["name"], H3))
        else:
            st += [Spacer(1, 3.5 * mm), Paragraph(company["name"], H3)]
        st += [Spacer(1, 1.1 * mm), kv(company["bullets"])]
        if company["note"]:
            st += [Spacer(1, 0.8 * mm), Paragraph(f"※ {_bold(company['note'])}", SMALL)]

        for case in company["cases"]:
            if case["vendor"]:
                # 孤立防止: 案件ブロック全体ではなく「見出し＋テーブル最初の1行」だけを
                # KeepTogetherする。ベンダーテーブルはヘッダ行が repeatRows で繰り返される
                # ため、そのまま独立して流し込んでよい
    ...
    cert += [Spacer(1, 0.9 * mm), mt, Spacer(1, 2.2 * mm),
             Paragraph(data["sn_heading"], H4), Spacer(1, 0.9 * mm)]

    TAG = S("tag", 8.6, leading=12.3)
    groups = list(data["sn_groups"]) + [(data["ms_heading"], data["ms_body"])]
    for h, b in groups:
        cert.append(KeepTogether([Paragraph(f"<b>{h}</b>", TAG), Paragraph(_bold(b), TAG),
                                  Spacer(1, 1.6 * mm)]))

    # 対応中の上位資格（結果待ち・受験予定）（CRMA・CTA等）。主要認定・ServiceNow認定とは別区分として明示的に描画
...
if __name__ == "__main__":
    for d in (OUT_MD, OUT_PDF):
        os.makedirs(d, exist_ok=True)

    suffixes = _discover_suffixes()
    if not suffixes:
        raise RuntimeError(
            "documents/output/md/ に 履歴書_*_ja.md 等が見つかりません。"
    ...
    for suffix in suffixes:
        ...
        if os.path.exists(paths["rirekisho"]):
            built.append(build_rirekisho(os.path.join(OUT_PDF, f"履歴書_{suffix}_ja.pdf"), suffix))
        if os.path.exists(paths["shokumu"]):
            built.append(build_shokumu(os.path.join(OUT_PDF, f"職務経歴書_{suffix}_ja.pdf"), suffix))
        if os.path.exists(paths["resume_en"]):
            built.append(build_resume_en(os.path.join(OUT_PDF, f"Resume_{suffix}_en.pdf"), suffix))
        if os.path.exists(paths["rirekisho_en"]):
            built.append(build_rirekisho_en(os.path.join(OUT_PDF, f"履歴書_{suffix}_en.pdf"), suffix))
